...
class DAOGenerator {
    ...
    public static function generateDAO($pdo, $dbName, $tableName, $options = "CRUD+OJAS12345") {
        ...
        $dao = "";
        $dao .= "<?php" . DAOGenerator::lineBreak(2);
        $dao .= "class " . DAOGenerator::underscoreToCamelCase($tableName, true) . " {" . DAOGenerator::lineBreak(2);
        if (strstr($options, "O")) {
            $dao .= DAOGenerator::propString($allCols) . DAOGenerator::lineBreak();
            $dao .= DAOGenerator::tab() . 'public function __construct(' . DAOGenerator::constructString($allCols) . ') {' . DAOGenerator::lineBreak();
            $dao .= DAOGenerator::feedString($allCols);
            $dao .= DAOGenerator::tab() . '}' . DAOGenerator::lineBreak(2);
            $dao .= DAOGenerator::getterString($allCols);
            $dao .= DAOGenerator::setterString($allCols);
            if (strstr($options, "J")) {
                $dao .= DAOGenerator::tab() . "public function toJSONString() {" . DAOGenerator::lineBreak();
                $dao .= DAOGenerator::tab(2) . "return(json_encode((array) \$this));" . DAOGenerator::lineBreak();
                $dao .= DAOGenerator::tab() . "}" . DAOGenerator::lineBreak(2);
            }
            if (strstr($options, "A")) {
                $dao .= DAOGenerator::tab() . "public function toArray() {" . DAOGenerator::lineBreak();
                $dao .= DAOGenerator::tab(2) . "return((array) \$this);" . DAOGenerator::lineBreak();
                $dao .= DAOGenerator::tab() . "}" . DAOGenerator::lineBreak(2);
            }
            if (strstr($options, "S")) {
                $dao .= DAOGenerator::toStringString($tableName, $allCols) . DAOGenerator::lineBreak(2);
            }
        }
        if (strstr($options, "C")) {
